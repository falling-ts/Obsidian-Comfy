...
插件按固定约定 `@{(数字)(名字)/(数字)(名字)}` 把原文补回：只把那个 `<em>` 换回字面文本 `_内容_`，单元格其余子节点一个不动；已经渲染正确的格子直接跳过，因此可重复执行。另外还校验那个 `<em>` 覆盖的正好是「名字/编号」那一段——格内本来就用 `*`/`_` 写的强调、或人名段内部含下划线的情况都不会被误补。
...
- **`@{...}` 下划线还原怎么同时覆盖预览与编辑**：实时预览的表格是 Obsidian 自己的 CM6 widget，它渲染单元格时会把单元格内容交给 `MarkdownPostProcessor` 注册表（`el` = 单元格内的 `.table-cell-wrapper`），所以不必写 CM6 装饰器，同一个后处理器在两种模式下都会被调用；还原只认「整格是 `@{数字…/数字…}` 且下划线确实被吃掉（存在唯一 `<em>`）」，对正常渲染与真正的斜体文本都不动手；并要求 `<em>` 覆盖的正好是「名字/编号」那一段，避免把格内真强调误判成被吃掉的下划线。
